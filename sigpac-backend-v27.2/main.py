        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if data_url:
                    ...
                    for e in estaciones:
                        try:
                            lat_s = str(e.get("latitud", "")).replace(",", ".")
                            lon_s = str(e.get("longitud", "")).replace(",", ".")
                            if "N" in lat_s or "S" in lat_s:
...
def _parse_dms(s: str) -> float:
    """Convierte formato AEMET tipo '412429N' a decimal."""
    s = s.strip()
    if not s:
        return 0.0
    neg = s[-1] in ('S', 'W', 'O')
    s = s[:-1] if s[-1] in ('N','S','E','W','O') else s
    s = s.replace(",",".")
    try:
        if "." in s:
